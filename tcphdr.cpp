// https://gitlab.com/gilgil/g/-/blob/master/src/net/pdu/gtcphdr.cpp
#include "tcphdr.h"

char* TcpHdr::parseData(TcpHdr* tcpHdr) {
	return reinterpret_cast<char*>(tcpHdr) + tcpHdr->off() * 4;
}

uint16_t TcpHdr::parseDataLen(IpHdr *ipHdr, TcpHdr* tcpHdr) {
    return ipHdr->len() - ipHdr->hl() * 4 - tcpHdr->off() * 4;
}
